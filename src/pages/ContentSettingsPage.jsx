...
function ProductEditor({ settings, edit }) {
  return (
    <Accordion className="home-settings-accordion" type="multiple" defaultValue={["hero"]}>
      ...
      <AccordionItem value="browser"><SectionHeader title="浏览与筛选区" description="筛选标题、结果文案、排序和空状态" enabled={settings.browser.enabled} onEnabled={(value) => edit((next) => { next.browser.enabled = value; })} /><AccordionContent><Card size="sm"><CardContent className="home-section-content"><div className="home-fields-grid"><TextControl id="browser-filter-title" label="筛选标题" value={settings.browser.filterTitle} onChange={(value) => edit((next) => { next.browser.filterTitle = value; })} /><TextControl id="browser-result-title" label="结果标题" value={settings.browser.resultTitle} onChange={(value) => edit((next) => { next.browser.resultTitle = value; })} description="使用 {count} 代表当前结果数量" /><TextControl id="browser-sort-label" label="排序文案" value={settings.browser.sortLabel} onChange={(value) => edit((next) => { next.browser.sortLabel = value; })} /><TextControl id="browser-clear-label" label="清除筛选文案" value={settings.browser.clearLabel} onChange={(value) => edit((next) => { next.browser.clearLabel = value; })} /></div><TextControl id="browser-empty-title" label="空状态文案" value={settings.browser.emptyTitle} onChange={(value) => edit((next) => { next.browser.emptyTitle = value; })} /><div className="home-fields-grid"><SelectControl id="browser-sort" label="默认排序" value={settings.browser.defaultSort} options={[["high", "价格从高到低"], ["low", "价格从低到高"]]} onChange={(value) => edit((next) => { next.browser.defaultSort = value; })} /><div><ToggleControl id="browser-filters" label="显示筛选栏" checked={settings.browser.showFilters} onChange={(value) => edit((next) => { next.browser.showFilters = value; })} /><ToggleControl id="browser-sort-enabled" label="显示排序控件" checked={settings.browser.showSort} onChange={(value) => edit((next) => { next.browser.showSort = value; })} /></div></div></CardContent></Card></AccordionContent></AccordionItem>
      <AccordionItem value="items"><SectionHeader title="产品卡片" description="图片、标签、名称、规格、价格与跳转链接" count={settings.items.length} /><AccordionContent><div className="home-item-list">{settings.items.map((item, index) => <ItemCard key={item.id} title={item.title} subtitle={`产品 ${index + 1}`} onDelete={() => edit((next) => { next.items.splice(index, 1); })}><div className="home-fields-grid"><TextControl id={`product-tag-${item.id}`} label="标签" value={item.tag} onChange={(value) => edit((next) => { next.items[index].tag = value; })} /><TextControl id={`product-name-${item.id}`} label="名称" value={item.title} onChange={(value) => edit((next) => { next.items[index].title = value; })} /><TextControl id={`product-location-${item.id}`} label="位置/分组" value={item.location} onChange={(value) => edit((next) => { next.items[index].location = value; next.items[index].locationGroup = value; })} /><TextControl id={`product-type-${item.id}`} label="类型" value={item.type} onChange={(value) => edit((next) => { next.items[index].type = value; })} /><TextControl id={`product-price-${item.id}`} label="价格文案" value={item.price} onChange={(value) => edit((next) => { next.items[index].price = value; })} /><TextControl id={`product-price-value-${item.id}`} label="排序价格数值" type="number" value={item.priceValue} onChange={(value) => edit((next) => { next.items[index].priceValue = Number(value) || 0; })} /><TextControl id={`product-beds-${item.id}`} label="规格一" value={item.beds} onChange={(value) => edit((next) => { next.items[index].beds = value; })} /><TextControl id={`product-baths-${item.id}`} label="规格二" value={item.baths} onChange={(value) => edit((next) => { next.items[index].baths = value; })} /><TextControl id={`product-area-${item.id}`} label="规格三" value={item.area} onChange={(value) => edit((next) => { next.items[index].area = value; })} /><TextControl id={`product-link-${item.id}`} label="跳转链接" value={item.link} onChange={(value) => edit((next) => { next.items[index].link = value; })} /></div><ImageControls prefix={`product-${item.id}`} image={item.image} position={item.imagePosition} onImage={(value) => edit((next) => { next.items[index].image = value; })} onPosition={(value) => edit((next) => { next.items[index].imagePosition = value; })} /><ToggleControl id={`product-enabled-${item.id}`} label="显示该产品" checked={item.enabled} onChange={(value) => edit((next) => { next.items[index].enabled = value; })} /></ItemCard>)}</div><Button variant="outline" size="sm" onClick={() => edit((next) => { next.items.push({ id: uid("product"), tag: "New", title: "新产品", location: "", locationGroup: "", price: "¥0", priceValue: 0, type: "Compute", beds: "0", baths: "0", area: "", features: [], image: "/images/hero-galaxy-home.png", imagePosition: "center center", link: "/estates", enabled: true }); })}><Plus />添加产品</Button></AccordionContent></AccordionItem>
