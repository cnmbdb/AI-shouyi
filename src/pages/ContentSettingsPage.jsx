  ...
  return (
    <Accordion className="home-settings-accordion" type="multiple" defaultValue={["brand", "items"]}>
      ...
      <AccordionItem value="browser-meta">
        ...
        <AccordionContent><Card size="sm"><CardContent className="home-section-content"><ImageControls prefix="navigation-favicon" label="浏览器图标（favicon）" image={settings.favicon} onImage={(value) => edit((next) => { next.favicon = value; })} variant="logo" placeholder={<BrandLogoMark fallbackClassName="navigation-logo-fallback-preview" />} /><TextControl id="nav-browser-title" label="浏览器标题" value={settings.browserTitle} description="显示在浏览器标签页、收藏夹和系统最近访问记录中。" onChange={(value) => edit((next) => { next.browserTitle = value; })} /></CardContent></Card></AccordionContent>
      </AccordionItem>
...
function FooterEditor({ settings, edit }) {
  ...
  return (
    <Accordion className="home-settings-accordion" type="multiple" defaultValue={["brand", "contact"]}>
      ...
      <AccordionItem value="contact"><SectionHeader title="联系信息与右侧展示图" description="电话、邮箱、地址，以及公开页脚右侧的大图" /><AccordionContent><Card size="sm"><CardContent className="home-section-content"><div className="home-fields-grid"><TextControl id="footer-contact-title" label="栏目标题" value={settings.contact.title} onChange={(value) => edit((next) => { next.contact.title = value; })} /><TextControl id="footer-phone" label="联系电话" value={settings.contact.phone} onChange={(value) => edit((next) => { next.contact.phone = value; })} /><TextControl id="footer-email" label="联系邮箱" value={settings.contact.email} onChange={(value) => edit((next) => { next.contact.email = value; })} /><TextControl id="footer-address" label="联系地址" textarea value={settings.contact.address} onChange={(value) => edit((next) => { next.contact.address = value; })} /></div><ImageControls prefix="footer" label="页脚右侧展示大图（替换公开页右侧图片）" image={settings.image} position={settings.imagePosition} onImage={(value) => edit((next) => { next.image = value; })} onPosition={(value) => edit((next) => { next.imagePosition = value; })} previewAspect="16 / 9" /></CardContent></Card></AccordionContent></AccordionItem>
...
function ProductHeroCardEditor({ card, index, edit }) {
  return (
    <ItemCard title={`轮播卡片 ${index + 1}`} subtitle="卡片图片、焦点与整卡跳转">
      <TextControl
      ...
      <ImageControls
        label="卡片图片"
        prefix={`product-hero-card-${card.id}`}
        image={card.image}
        position={card.imagePosition}
        onImage={(value) => edit((next) => { next.hero.cards[index].image = value; })}
        onPosition={(value) => edit((next) => { next.hero.cards[index].imagePosition = value; })}
        previewAspect="1086 / 1448"
      />
    </ItemCard>
  );
}
