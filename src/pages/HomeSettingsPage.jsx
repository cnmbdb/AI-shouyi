    ...
    <div className="home-settings-page">
      <Accordion className="home-settings-accordion" type="multiple" defaultValue={["hero"]}>
        ...
        <AccordionItem value="features">
          ...
          <AccordionContent><Card size="sm"><CardContent className="home-section-content"><div className="home-fields-grid"><SelectControl id="features-mobile-columns" label="移动端每行卡片数" value={settings.features.mobileColumns} options={[[2, "每行 2 张"], [1, "每行 1 张"]]} onChange={(value) => setField("features", "mobileColumns", Number(value))} /><TextControl id="features-mobile-height" label="移动端卡片高度（px）" type="number" value={settings.features.mobileCardHeight} description="范围 180–420；双列时建议 220–280。" onChange={(value) => setField("features", "mobileCardHeight", Number(value))} /></div></CardContent></Card><div className="home-item-list">{settings.features.items.map((item, index) => <ItemCard key={item.id} title={item.title} subtitle={`卡片 ${index + 1}`} onDelete={() => removeItem("features", "items", index)}><TextControl id={`feature-title-${item.id}`} label="标题" value={item.title} onChange={(value) => setItem("features", "items", index, "title", value)} /><TextControl id={`feature-copy-${item.id}`} label="文案" textarea value={item.description} onChange={(value) => setItem("features", "items", index, "description", value)} /><ImageControls prefix={`feature-${item.id}`} image={item.image} position={item.imagePosition} onImage={(value) => setItem("features", "items", index, "image", value)} onPosition={(value) => setItem("features", "items", index, "imagePosition", value)} previewAspect="1 / 1" /><LinkControl id={`feature-link-${item.id}`} value={item.link} onChange={(value) => setItem("features", "items", index, "link", value)} /></ItemCard>)}</div><Button variant="outline" size="sm" onClick={() => addItem("features", "items", { icon: "GraphicsCard", title: "新卡片", description: "", image: "/images/hero-galaxy-home.png", imagePosition: "50% 50%", link: "/estates" })}><Plus />添加卡片</Button></AccordionContent>
        </AccordionItem>
