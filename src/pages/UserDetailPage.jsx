...
export function UserDetailPage({ userId, currentUser, onNavigate, onNotice }) {
  ...
  const orderIdsWithDevices = useMemo(() => new Set((detail?.devices ?? []).map((device) => device.store_order_id).filter(Boolean)), [detail?.devices]);
  const paidTotal = (detail?.orders ?? []).filter((order) => completedStatuses.has(order.status) && order.product_snapshot?.source !== "admin").reduce((sum, order) => sum + Number(order.total_amount), 0);
  ...
  const recordMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      onNotice?.(result?.source === "管理员添加" ? `管理员订单 ${result.orderNo} 与 ${result.quantity} 台设备已生成` : "用户业务资料已更新");
    },
  ...
  const saveEditor = (values) => recordMutation.mutate({
    userId,
    resource: editor.kind === "renewal" ? "device" : editor.kind,
    operation: editor.kind === "renewal" ? "renew" : editor.record ? "update" : "create",
  ...
  const orderById = new Map((detail.orders ?? []).map((order) => [order.id, order]));
      ...
      <Tabs defaultValue="orders" className="admin-user-detail-tabs">
        ...
        <TabsContent value="orders">
          <Card><CardHeader className="border-b"><CardTitle>商城订单</CardTitle><CardDescription>新增订单必须连接商品列表中的真实商品，支付和续费关联存在时禁止删除。</CardDescription><CardAction><Button size="sm" onClick={() => setEditor({ kind: "order", record: null })}><PlusIcon />新增订单</Button></CardAction></CardHeader><CardContent className="admin-user-detail-table"><Table><TableHeader><TableRow><TableHead>订单号</TableHead><TableHead>商品</TableHead><TableHead>方式</TableHead><TableHead>金额</TableHead><TableHead>支付</TableHead><TableHead>状态</TableHead><TableHead>下单时间</TableHead><TableHead>操作</TableHead></TableRow></TableHeader><TableBody>
            {(detail.orders ?? []).map((order) => { const product = productMap.get(order.product_id); const payment = paymentByOrder.get(order.id); const adminAdded = order.product_snapshot?.source === "admin"; return <TableRow key={order.id}><TableCell><strong>{order.order_no}</strong>{adminAdded ? <Badge variant="outline" className="admin-source-badge">管理员添加</Badge> : null}</TableCell><TableCell><div className="admin-order-product"><span>{product?.name ?? order.product_snapshot?.name ?? "已删除商品"}</span><small>{product?.sku ?? order.product_snapshot?.sku ?? order.product_id}</small></div></TableCell><TableCell>{orderTypeLabel[order.order_type] ?? order.order_type}{order.period_count ? <small className="admin-order-period">{order.period_count} {periodUnitLabel[order.period_unit] ?? order.period_unit}</small> : null}</TableCell><TableCell>{money(order.total_amount)}</TableCell><TableCell>{payment ? <><StatusBadge value={payment.status} /><small className="admin-order-period">{payment.payment_no}</small></> : adminAdded ? <Badge variant="secondary">无需支付</Badge> : "-"}</TableCell><TableCell><StatusBadge value={order.status} /></TableCell><TableCell>{dateTime(order.created_at)}</TableCell><TableCell><div className="admin-row-actions">{product ? <Button variant="ghost" size="icon-xs" aria-label="查看商品" onClick={() => openProduct(order)}><ExternalLinkIcon /></Button> : null}<Button variant="ghost" size="icon-xs" aria-label="调整订单" onClick={() => setEditor({ kind: "order", record: order })}><PencilIcon /></Button><Button variant="destructive" size="icon-xs" aria-label={orderIdsWithDevices.has(order.id) ? "订单已有设备，不能删除" : "删除订单"} title={orderIdsWithDevices.has(order.id) ? "订单已生成设备，请停用设备并保留历史" : "删除订单"} disabled={orderIdsWithDevices.has(order.id)} onClick={() => setDeleteTarget({ resource: "order", targetId: order.id, label: order.order_no })}><Trash2Icon /></Button></div></TableCell></TableRow>; })}
        ...
        <TabsContent value="products">
          ...
          <Card className="admin-user-device-card"><CardHeader className="border-b"><CardTitle>历史已购设备与当前资产</CardTitle><CardDescription>添加设备会从真实商城商品与 SKU 生成管理员订单并扣减对应库存；启停只改变运行状态，续费会创建关联订单。</CardDescription><CardAction><Button size="sm" onClick={() => setEditor({ kind: "device", record: null })}><PlusIcon />从商城添加设备</Button></CardAction></CardHeader><CardContent className="admin-user-detail-table"><Table><TableHeader><TableRow><TableHead>设备编号</TableHead><TableHead>设备 / 来源订单</TableHead><TableHead>算力</TableHead><TableHead>状态</TableHead><TableHead>日收益</TableHead><TableHead>到期日</TableHead><TableHead>操作</TableHead></TableRow></TableHeader><TableBody>{(detail.devices ?? []).map((device) => { const sourceOrder = orderById.get(device.store_order_id); const adminAdded = sourceOrder?.product_snapshot?.source === "admin"; return <TableRow key={device.id}><TableCell>{device.device_code}</TableCell><TableCell><div className="admin-order-product"><span>{device.name}</span><small>{sourceOrder ? `${adminAdded ? "管理员添加 · " : ""}订单 ${sourceOrder.order_no}` : "历史手工设备"}</small></div></TableCell><TableCell>{device.compute}</TableCell><TableCell><StatusBadge value={device.status} /></TableCell><TableCell>{money(device.daily_yield)}</TableCell><TableCell>{dateOnly(device.expires_at)}</TableCell><TableCell><div className="admin-row-actions">{device.store_order_id && sourceOrder?.order_type === "rental" ? <Button variant="outline" size="xs" onClick={() => setEditor({ kind: "renewal", record: device })}><CalendarClockIcon />续费</Button> : null}<Button variant="ghost" size="icon-xs" aria-label={device.status === "已停用" ? "启用设备" : "停用或调整设备"} onClick={() => setEditor({ kind: "device", record: device })}><PencilIcon /></Button><Button variant="destructive" size="icon-xs" aria-label="删除设备" onClick={() => setDeleteTarget({ resource: "device", targetId: device.id, label: device.device_code })}><Trash2Icon /></Button></div></TableCell></TableRow>; })}{!(detail.devices ?? []).length ? <TableRow><TableCell colSpan={7}><EmptyState>暂无历史购买或设备资产</EmptyState></TableCell></TableRow> : null}</TableBody></Table></CardContent></Card>
        </TabsContent>
