...
Prototype calculator commerce-source rule: the public yield calculator reads the administrator-selected subset of enabled products and their live run-compute specifications from the commerce catalog rather than persisting a second set of calculator plans. Calculator settings provide an existing-product multi-select with select-all and clear-all actions; an omitted selection means all enabled products, while an explicit selection controls the public product list. Every product has one unique GPU model and an administrator-managed mall-style SKU framework split into two blocks. The specification block contains up to four freely editable dimensions—compute power, monthly rental return percentage, rental duration, and daily TOKEN output—and every dimension supports any number of add/delete values. The price block is not a fifth specification dimension: it automatically generates the Cartesian product of all four dimensions, shows one row per SKU combination, and gives every row its own editable monthly rental price and per-SKU inventory (for example, three values in each dimension generate 81 price-and-stock rows). Specification option changes regenerate the matrix while preserving prices and inventory for unchanged combinations. The editor hides the internal specification-type selector; administrators edit each dimension's display name and unit directly. The calculator, product detail, trusted order repricing, inventory checks, payment completion, and immutable order snapshot share the same selected SKU, price, and stock; Calculator CMS retains presentation copy and product visibility selection while specification dimensions, options, SKU prices, and SKU inventory are edited under Commerce products.
...
Prototype mobile console-sidebar rule: the console drawer keeps the brand fixed while its navigation is a dedicated touch-scroll region. Every administrator group and site-setting entry, including pages added below the initial viewport, must remain reachable above the device safe area and mobile browser toolbar.

Prototype console-help rule: do not render a help-ticket or contact-support card at the bottom of the authenticated console sidebar.
...
Prototype homepage hero responsive-CMS rule: Home Hero settings expose independent desktop and mobile background images, heights, contain/cover fit, two-axis crop focal points, and 100–250% crop zoom. Mobile media falls back to the desktop image when blank. Crop previews support direct pointer dragging plus numeric and keyboard focal adjustment, and the published Hero renders the matching device configuration without a gradient shadow overlay.

Prototype homepage hero resolution rule: the bundled compute Hero is a true 2560px-wide asset with 768px, 1280px, 1920px, and 2560px responsive WebP delivery. Desktop high-density displays must be allowed to select the 2K candidate instead of being capped at 1280px, and the known legacy 1448px uploaded Hero normalizes to this bundled 2K replacement.

Prototype homepage feature-card rule: the four Home feature cards render without a leading icon or icon container on desktop and mobile. Their public anatomy is background image, title, description, and the existing lower-right navigation arrow; Home settings do not expose an icon selector for these cards.
...
Prototype homepage mobile hero-fit rule: the mobile shared header must always span the full viewport and must not inherit the narrowed `.shell` width. The bundled 4:3 Home Hero defaults to a 75% height-to-width section, starts immediately below the 58px shared header so its upper artwork is never covered, uses a white fallback surface for contain-fit media, and the following feature grid starts after the Hero rather than overlapping it with a negative margin.

Prototype store-product color rule: public commodity detail pages use NVIDIA Green `#76B900` as their only interaction and selection accent. Product specification icons, selected billing plans, primary purchase actions, sharing controls, focus rings, detail labels, borders, and empty-state actions must use the green and neutral daytime system; do not reintroduce purple or blue-purple chrome.
