...
Prototype product hero surface rule: the public product-browsing Hero displays only the five-card carousel, with no title, description, or breadcrumb. Its surface is transparent over a white product-page background in light theme and a black product-page background in dark theme. Product Hero settings expose only media, focal-position, visibility, and upload controls; copy and breadcrumb fields are not persisted.

Prototype product hero theme-media rule: the product Hero uses `/images/gpu2.png` for all five cards in light theme and the configured `/images/gpu-carousel-card.png`-compatible source in dark theme. The bundled light-theme asset is cropped tightly to 986 × 1410 at the outside edge of its rounded pink card border, including removal of the residual right and bottom white strips, and the carousel uses its true card aspect ratio so the border is not clipped. Product Hero settings expose separate image, focal-position, and upload controls for the light and dark theme card artwork.

Prototype product hero CMS rule: Product browsing settings manage the complete five-card Hero. Administrators can enable or hide the section, configure desktop maximum height and mobile height, desktop and mobile primary-card widths, the autoplay interval, and each of the five cards' light-theme image, dark-theme image, focal positions, uploads, and whole-card click link. Saving normalizes numeric limits and publishes the configuration directly to the public `/estates` Hero while remaining read-compatible with the legacy shared Hero image fields.
