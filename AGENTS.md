...
Prototype product hero theme-media rule: the product Hero uses `/images/gpu2.png` for all five cards in light theme and the configured `/images/gpu-carousel-card.png`-compatible source in dark theme. The bundled light-theme asset is cropped tightly to the outside edge of its rounded pink card border with no rectangular white margin, and the carousel uses its true card aspect ratio so the border is not clipped. Product Hero settings expose separate image, focal-position, and upload controls for the light and dark theme card artwork.
